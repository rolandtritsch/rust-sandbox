...
[guessing-game.md--expect]: http://doc.rust-lang.org/std/option/enum.Option.html#method.expect
...
[the heap][ownership.md--heap] for each of its elements. When `v` goes out of scope at the
...
contains. The vector object is stored on the [stack][ownership.md--sh] and contains a pointer
to the content (`[1, 2, 3]`) stored on the [heap][ownership.md--sh]. When we move `v` to `v2`,
